...
class OrderController extends Controller {
    ...
    Public function xiangqing(){
        ...
        // $mod->query('');
        // $page->firstRow分页的起始总行数 分页的偏移量 $page->listRows每页数据的大小
        $list=$mod->join('orders on orderdetail.order_id=orders.id')->where($where)->limit($page->firstRow,$page->listRows)->field('')->select();
         // var_dump($list);exit;
        ...
        foreach($list as $row){
            // var_dump($row);exit;
             $data=array();
             $data['pic']=$row['pic'];
             $data['type']=$row['shop_type'];
             $data['color']=$row['shop_color'];
             $data['num']=$row['shop_num'];
             $data['price']=$row['shop_price'];
             $data['number']=$row['number'];
             $data['addtime']=$row['addtime'];
             $data['total']=$row['total'];
             $data['status']=$row['status'];
             $data['liuyan']=$row['liuyan'];
        ...
        //实例化地址Model类
        $mod2=M('address');
        $info =$mod2->where('id','=',$id)->select();
        // var_dump($info);exit;
       foreach($info as $rr){
            // var_dump($rr);exit;
            $data['name']=$rr['name'];
            $data['phone']=$rr['phone'];
            $data['address']=$rr['address'];
            $data['code']=$rr['code'];
       }
        // var_dump($data);exit;
